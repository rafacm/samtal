...
- **Getting Started ends by saying how to change the parts.** The
  ...
  reader arrives at them. `Swede it yourself` names them per stage,
  the model, transcription, voice and MCP tools, one emoji-led bullet
  each in the Features section's shape, with the two presets and the
  generated field reference behind them, and states the rule that a
