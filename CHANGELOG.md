...
- The architecture walkthrough caught up with the pipeline it describes: the transcription step names the OpenAI option beside local faster-whisper, the synthesis step names OpenAI and ElevenLabs beside Piper and explains why the next sentence is synthesized while the current one plays, and the closing step says what hangs up a realtime conversation nobody came back to. A new closing section names the two things a happy-path picture cannot show, interrupting a reply and which stages leave the host, and points at the PlantUML diagram for each.
