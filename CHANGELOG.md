...
- **The tests reach for the names a caller reaches for, and the ones
  ...
  name through a dot: 440 sites over 85 names across 55 files. 133 of
  ...
  and the `tts sentence_start` message a device already receives. 10
  asserted nothing that survived and are gone. The remaining 297 are
  white-box safety invariants and each now states, where it happens,
  which property public observation cannot establish: a race that is
  only ever probabilistically reproducible, a released resource that is
  invisible because it was released, a token older than an issuer will
  stamp, a stored column a migration reads rather than an accessor. The
  walk after the sweep reports 167 sites over 77 names across 48 files.
  No production interface was added, and no production code changed.
