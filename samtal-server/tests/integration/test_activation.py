...
from samtal_server.onboarding import onboarding_key, onboarding_path
...
async def test_a_board_is_onboarded_by_the_code_it_shows(
    ...
    async with serve_app_in(directory, CONFIG) as (port, app):
        ...
        # board does with it.
        short = onboarding_path(onboarding_key(app.state.config.server))
    ...
    assert onboarding_path(onboarding_key(restarted.state.config.server)) == short
