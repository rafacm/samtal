...
rather than in front of it. Two things about that are only true in a real
process: uvicorn renders a lifespan exception by formatting its whole
traceback into a log line, and the exit code is set by `main()` after
`serve()` has already returned. Neither can be checked from a test that
...
`serve()`. What must not come out is anything the failure was chained
from: a provider failure this deep carries a driver's or a client
library's own exception, and those quote the URL or the credential they
were configured with. So the refusal here is chained from one that
carries a credential-shaped sentinel, and the sentinel is hunted for in
...
def run_entrypoint(tmp_path: Path) -> subprocess.CompletedProcess[str]:
    """One server process, on a configuration whose providers refuse.
...
def test_a_refused_startup_says_one_sentence_and_exits_one(tmp_path: Path) -> None:
    finished = run_entrypoint(tmp_path)
    written = finished.stdout + finished.stderr

    assert finished.returncode == 1, written
    assert SENTENCE in finished.stderr, written
...
def test_nothing_the_refusal_was_chained_from_reaches_either_stream(
    ...
    the traceback uvicorn formats has nothing to walk into."""
    finished = run_entrypoint(tmp_path)
    written = finished.stdout + finished.stderr

    assert SENTINEL not in written, written
    assert "api.example" not in written, written
    # And what was written is still a usable diagnostic rather than a
    # silence: the class name says a startup refused, and the sentence
    # says why.
    assert "StartupFailed" in written
