...
Opt in with SAMTAL_SMOKE_OTA_URL, the same pattern as the local lane, so
a bare `pytest` never picks this up. SAMTAL_AUTH_SECRET must be the
...
OTA_URL_ENV = "SAMTAL_SMOKE_OTA_URL"
...
def device_auth(ota_url: str):
    ...
    from samtal_server.config.models import AuthConfig
