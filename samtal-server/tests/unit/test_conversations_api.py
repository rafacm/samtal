...
from samtal_server.config.api import MOUNT_PATH, UNAUTHORIZED, UNEXPECTED, build_api
from samtal_server.conversations import api as conversations_api
from samtal_server.conversations import schema
from samtal_server.conversations.api import (
    CURSOR_REFUSED,
    DEVICE_REFUSED,
    LIMIT_DEFAULT,
    LIMIT_MAX,
...
from tests.unit.test_conversations_session import until
...
def _leaked(caplog: pytest.LogCaptureFixture) -> str:
    ...
    """
    records = [record for record in caplog.records if record.name.startswith("samtal_server")]
    return "".join(
