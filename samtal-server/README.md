...
`/api` on its own port. It is what `samtal-server config` talks to, and
...
`samtal-server config openapi` and regenerated by CI, so it cannot drift
...
`server.api.secret_env` names, `SAMTAL_API_SECRET` by default, and a
server started without it refuses to boot, naming the variable and the
fix:

```bash
SAMTAL_API_SECRET=$(openssl rand -hex 32)
```
...
3. `http://127.0.0.1:<server.port>/api`, the port read from the same
   ...
   `SAMTAL_CONFIG`), so the two cannot disagree about it
...
| `llm_round`        | a generation call finishes      | `agent`, `round`, `turns`, `duration_ms`, `stage`, `provider`, `type`, `host`, `first_token_ms` (the first spoken token, so a round that only called a tool carries none), plus `input_tokens` and `output_tokens` where the provider reports them |
...
`samtal-server config pending` lists every board waiting, with the board
