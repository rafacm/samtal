...
- The exact vocabulary of the two landed surfaces:
  ...
  stale here.
- Still open, each with its owner: resuming a thread and the recap
  checkpoints of one (#190, the two milestones after the read verbs),
  exporters over the same tap and vocabulary (#66/#67), the audit
