...
**AEC (acoustic echo cancellation).** Signal processing that removes
...
**Agent.** One configured persona: a system prompt, an LLM, a voice,
...
**ASR (automatic speech recognition).** The stage that turns one
...
**ASR prompt.** Free-text context an ASR accepts to bias
...
**Barge-in.** Interrupting the assistant by talking over its reply.
...
**Capture.** Per-session recording for field analysis: a stereo WAV
...
**Conversational filler.** A short utterance that buys time in a
...
**Cross-correlation.** Sliding one signal against another and
...
**Echo leakage.** How much of the assistant's own voice survives the
...
**Endpointer.** The logic that decides where an utterance ends:
...
**First token.** The first piece of an LLM's streamed answer; the
...
**Handover.** Switching the active agent mid-session, requested by
...
**Idle timeout.** A realtime session with no conversation for the
...
**Listening modes.** How the device decides when the microphone is
...
**LLM round.** One generation call within a reply. Tool use makes a
...
**Manifest.** The JSON sidecar of a capture: device, firmware,
...
**Marker phrase.** A fixed, otherwise-improbable phrase ("banana,
...
**MCP (Model Context Protocol).** The protocol samtal uses for tools
...
**Opus.** The audio codec on the device link, carrying 60 ms frames
...
**OTA endpoint.** The HTTP endpoint a device calls at boot with its
...
**PCM.** Raw uncompressed audio samples, the working format between
pipeline stages once Opus is decoded: 16-bit mono, 16 kHz on the
input side.

**Pre-roll.** A short stretch of audio kept from before the VAD's
speech onset when trimming an utterance, so a soft first syllable is
not clipped by the trim.

**Prompt echo.** An ASR failure mode where the model returns the
...
**Refractory period.** The window right after the assistant starts
...
**Sentence lookahead.** Synthesizing the next sentence while the
...
**Trailing silence.** The stretch of non-speech the endpointer
...
**TTS (text-to-speech).** The stage that turns reply text into
...
**Utterance.** One stretch of user speech as delimited by the
endpointer: the unit ASR transcribes and the unit the conversation
advances by.

**VAD (voice activity detection).** Frame-by-frame classification of
...
**Wake word.** An always-on, on-device trigger phrase that opens a
