...
8. **The unit lane keeps its speed with one worker database and
   ...
   (`vinga_test_w{n}`, plain `vinga_test` serially), created and
   migrated once per worker from a template database
   (`vinga_test_template`) that the first worker builds under the
   same advisory-lock discipline the product uses; between tests
   it issues one `TRUNCATE ... RESTART IDENTITY CASCADE` across
   both schemas' tables, which is milliseconds, preserves the
   fresh-database-per-test property the throwaway directories
   gave, and keeps re-opens cheap (an already-migrated database is
   a version-table check, exactly as today). Tests whose subject
   is migration or fresh-boot behavior (`test_db_open`,
   `test_conversations_boot`, the integration recovery case) take
   an opt-in fixture that clones a throwaway database from the
   template (`CREATE DATABASE ... TEMPLATE`, dropped with
   `FORCE`). The lane refuses at session start with one sentence
