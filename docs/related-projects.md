...
The projects samtal is measured against, and the ones it is made of. This
is a survey of the neighbourhood, not a scoreboard: most of these predate
samtal, several are better at what they set out to do, and two of them
supply components samtal ships.
...
choose *instead of* samtal, and each one answers four questions in the
same order: what it is, where it overlaps, where samtal is deliberately
different, and what samtal borrows. [What samtal is built
from](#what-samtal-is-built-from) is the shorter register: what the thing
is and why samtal touches it. License terms for anything shipped or
...
model in the loop and no dialogue state. samtal's middle stage is an LLM
...
**What samtal takes.** Two of samtal's pipeline components come out of
this project. Piper, the local speech synthesis provider, is a Rhasspy
project, and samtal still downloads its voices from the
`rhasspy/piper-voices` model repository; `rhasspy/piper` was archived in
the same month as Rhasspy itself, so samtal uses the maintained
`piper1-gpl` successor. `pysilero-vad`, the voice activity detection in
every samtal conversation and a core dependency rather than an optional
one, is also published from the `rhasspy` organisation. samtal runs none
...
That stack, rather than Rhasspy 2.5, is samtal's nearest living
...
- **The hub.** Assist assumes Home Assistant as the conversation agent
  and the reason the device exists. samtal assumes no hub: the agent is
  configured in samtal's own config, and home automation is one MCP tool
  server among any others, or absent.
- **The wire.** Wyoming is a protocol its own ecosystem defines and can
  change. samtal speaks the xiaozhi device protocol, which buys the
  ...
  cost of implementing a protocol samtal does not control. That trade,
...
**Overlap.** The closest match to samtal's physical shape of anything in
...
which is what buys its latency and its short setup. samtal splits the
...
upstream project samtal tries not to fork.

**What samtal takes.** Nothing in code. It is worth reading as the
strongest argument for the road samtal did not take, single-vendor
realtime speech-to-speech, which is a plausible future samtal provider
...
**Overlap.** Almost none at the layer samtal occupies, despite the shared
...
hardware. Everything samtal spends its effort on lives below where Hermes
Agent starts.
...
candidate to sit behind samtal rather than beside it. samtal is the ears,
...
server samtal attaches, or as the endpoint samtal's LLM provider points
at.

**The tension to watch.** samtal's own direction (per-device agents,
household users, budgets) overlaps with what Hermes Agent calls memory
and user modelling. That is an argument for keeping samtal's agent layer
thin and delegating upward, not for rebuilding it.

## What samtal is built from

Not neighbours: the projects samtal actually runs, links, or drives. The
preference throughout is to use a published artifact rather than fork
one, so most of these are ordinary dependencies with no samtal patches.
...
audio pipeline, wake word, display UI, and the protocol samtal
implements.
[xinnan-tech/xiaozhi-esp32-server](https://github.com/xinnan-tech/xiaozhi-esp32-server)
is the Python conversation server samtal starts from and reimplements
...
- Cloud providers reached over their APIs rather than bundled:
  ...
  over plain HTTP.
- `edge-tts`, listed here for the opposite reason: it is GPL-3.0, samtal
...
samtal uses it as a device simulator to drive the server end to end
without a board on the desk (see the testing section of
[`samtal-server/README.md`](../samtal-server/README.md)). It is a
...
- The vendor realtime speech-to-speech APIs themselves (OpenAI Realtime,
  ...
  above treats as one architecture and samtal would treat as one
  provider.
