...
from vinga_server.config.api import MOUNT_PATH, NO_STORED_WORLD
from vinga_server.config.entities import SNAPSHOT_NOTICE
...
def test_a_device_write_says_it_is_stored_and_waits_for_a_start(
    ...
    assert bound.json() == {
        "wrote": f"device {DEVICE} bound to assistant",
        "notice": SNAPSHOT_NOTICE,
    }
    # Stored, which is the half the sentence promises: a server started
    # from this directory would read it.
...
def test_every_live_write_carries_the_same_sentence(client: TestClient) -> None:
    """All four of the writes that are otherwise answered as live, since
    a sentence right about one of them and wrong about the rest would be
    ...
    for answer in answers:
        ...
        assert answer.json()["notice"] == SNAPSHOT_NOTICE
...
def test_a_server_reading_a_store_answers_all_three_as_usual(
    ...
    assert bound.json()["notice"] != SNAPSHOT_NOTICE
