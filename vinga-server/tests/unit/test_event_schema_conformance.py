...
SPREAD_INVENTORY: dict[str, Spread] = {
    ...
    "vinga_server.providers.openai_asr:OpenAiAsr._echo_fields": Spread(
        LOCAL,
        alternatives=sets(
    ...
    "vinga_server.ota.reply:check_version.fields": Spread(
        LOCAL,
        alternatives=sets(
...
CALL_ALTERNATIVES: dict[tuple[str, str, int], tuple[frozenset[str], ...]] = {
    ("vinga_server.ota.reply", "check_version", 1): sets(
        "device client board firmware agents unloaded code"
    ),
    ("vinga_server.ota.reply", "check_version", 2): sets(
        "device client board firmware agents unloaded"
    ),
    ("vinga_server.ota.reply", "check_version", 3): sets(
        "device client board firmware agents unloaded"
    ),
    ("vinga_server.ota.reply", "check_version", 4): sets(
        "device client board firmware agents unloaded"
    ),
    ("vinga_server.providers.openai_asr", "OpenAiAsr._retry_without_prompt", 1): sets(
        "outcome duration_s host"
    ),
    ("vinga_server.providers.openai_asr", "OpenAiAsr._retry_without_prompt", 2): sets(
        "outcome duration_s host retry_ms"
    ),
    ("vinga_server.providers.openai_asr", "OpenAiAsr._retry_without_prompt", 3): sets(
        "outcome duration_s host retry_ms"
    ),
    ("vinga_server.providers.openai_asr", "OpenAiAsr._retry_without_prompt", 4): sets(
        "outcome duration_s host retry_ms"
    ),
    ("vinga_server.providers.openai_asr", "OpenAiAsr._retry_without_prompt", 5): sets(
        "outcome duration_s host retry_ms"
    ),
}
...
TOKEN_SOURCES: dict[tuple[str, str], tuple[Decides, ...]] = {
    ("capture_failed", "reason"): (
        Decides("vinga_server.capture", ARGUMENT, "_disable:0", scope="SessionCapture"),
    ),
    ...
    ("activation_not_offered", "reason"): (
        Decides("vinga_server.ota.reply", KEYWORD, "reason", scope="_activation"),
        Decides(
            "vinga_server.onboarding.pending",
            KEYWORD,
            "Offer:refused",
    ...
    ("activation_refused", "reason"): (
        Decides("vinga_server.ota.poll", KEYWORD, "reason", scope="_version_two"),
    ),
    ("ota_request_rejected", "arg:0"): (
        Decides("vinga_server.ota.reply", ARGUMENT, "_bad_request:0"),
    ),
    ("onboarding_banner", "origin_source"): (
        Decides(
            "vinga_server.onboarding.origin", ARGUMENT, "Origin:1", scope="public_origin"
        ),
    ),
    ("mcp_connected", "transport"): (
        Decides("vinga_server.config.models", ANNOTATION, "McpServerConfig.transport"),
    ),
    ...
    ("mcp_down", "reason"): (
        Decides("vinga_server.tools.mcp.transport", RETURNED, "_down_reason"),
        Decides("vinga_server.tools.mcp.transport", ARGUMENT, "reached:0"),
        Decides(
        ...
        Decides("vinga_server.tools.mcp.transport", CONSTANT, "STOPPED"),
        Decides("vinga_server.tools.mcp.transport", CONSTANT, "CALL_FAILED"),
    ),
    ("mcp_reload", "outcome"): (
        Decides("vinga_server.tools.mcp.reload", CONSTANT, "APPLIED"),
        Decides("vinga_server.tools.mcp.reload", CONSTANT, "REFUSED"),
    ),
    ("mcp_reload", "reason"): (
...
PINNED_BY: dict[tuple[str, str, int], tuple[str, ...]] = {
    ("vinga_server.app", "_build_composition", 1): (f"{SERVER_PINS}::test_capture_enabled",),
    ("vinga_server.app", "_build_composition", 2): (f"{SERVER_PINS}::test_capture_disabled",),
    ...
    ("vinga_server.ota.poll", "activate", 1): (f"{SERVER_PINS}::test_activation_complete",),
    ("vinga_server.ota.poll", "activate", 2): (f"{SERVER_PINS}::test_activation_pending",),
...
def test_the_narrowings_are_the_two_the_source_hides() -> None:
    ...
    assert {identity[:2] for identity in CALL_ALTERNATIVES} == {
        ...
        ("vinga_server.providers.openai_asr", "OpenAiAsr._retry_without_prompt"),
    }
...
# nothing said.
OUTCOME_LITERAL = ("vinga_server.onboarding.unbound", "Unbound", "outcome")

OUTCOME_NARRATOR = ("vinga_server.ota.reply", "_activation")
...
def test_a_submodule_may_not_emit_on_a_channel_nobody_owns_for_it() -> None:
    ...
    borrowed = trees({"planted.part": "from vinga_server.tools.mcp import events\n"})
    silent = trees({"planted": "\n", "planted.part": PACKAGES_EMITTER})
    astray = trees({"planted": OWN_EMITTER, "planted.part": PACKAGES_EMITTER})

    assert unowned("planted.part", "vinga_server.tools.mcp", borrowed)
...
def test_a_packages_init_is_named_for_the_package_it_defines() -> None:
    ...
    assert module_name(PACKAGE / "runtime" / "__init__.py") == "vinga_server.runtime"
    assert module_name(PACKAGE / "runtime" / "pipeline.py") == "vinga_server.runtime.pipeline"
    assert module_source("vinga_server.runtime") == (
...
def test_a_lawful_configured_name_passes_the_configuration(name: str) -> None:
    ...
    from vinga_server.config.models import NonBlankStr
