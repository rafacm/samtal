...
async def scripted(**options: object) -> MockLlm:
    config = ProviderConfig.model_validate({"type": "mock", **options})
    provider = await build_entry("llm", "mock", config)
...
async def test_without_tool_options_the_mock_only_speaks() -> None:
    llm = await scripted(reply="You said {text}.")
...
async def test_the_round_after_the_results_speaks_them() -> None:
    llm = await scripted(reply="The tool said {tool_result}.", tool_when="secret", tool_name="t")
    turns = [
...
async def test_an_untriggered_utterance_never_calls_a_tool() -> None:
    llm = await scripted(reply="You said {text}.", tool_when="secret", tool_name="t")
...
async def test_a_forbidden_call_is_not_made() -> None:
    ...
    llm = await scripted(reply="Fine, {tool_result}then.", tool_when="secret", tool_name="t")
    collected = await events(llm, [Turn("user", "the secret")], tool_choice="none")
    assert spoken(collected) == "Fine, then."


async def test_a_tool_arguments_option_that_is_not_a_mapping_is_refused() -> None:
    try:
        await scripted(tool_when="x", tool_name="t", tool_arguments="volume=5")
...
async def test_the_reply_can_say_which_tools_it_was_offered() -> None:
    ...
    llm = await scripted(reply="I have {tools}.")
