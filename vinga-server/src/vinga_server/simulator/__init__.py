...
`conversation` is the websocket half: the handshake, the hello exchange
and the ordering machine one turn passes through. `utterance` is the one
thing this board can say, encoded once at build time and carried in
`data/`. `capabilities` is the honest statement of what this simulator
does and does not do, declared once and rendered into the help so that
nobody debugs it believing it is a board.
...
following silently: `conversation` is the only module in this tree that
imports `websockets`, and an `__init__` re-exporting it would drag that
import into every import of this package, which is the extra's gate
defeating itself.
"""
