...
def reference() -> str:
    ...
    lines = [
        ...
        *_paragraph(
            ...
            "deployment that has to erase something now stops the server and deletes "
            "three files rather than one: `conversations.db`, and the "
            "`conversations.db-wal` and `conversations.db-shm` sidecars beside it. "
            "The log is not a cache of the database. It is where a committed row's "
            "bytes live until a checkpoint folds them back, so a deployment that "
            "removed the database alone would leave what it meant to erase sitting "
            "in the file next to it. That takes every session rather than the one "
            "that was asked about. Whichever way rows go, the deletion the "
        ...
        *_paragraph(
            ...
            "in the write-ahead log. Two limits are stated rather than implied. A "
            "checkpoint a reader is blocking does not fail the deletion: the rows "
            "are committed, the truncation is owed, and it is retried at the next "
            "quiet moment, which is the server's next write. And copies that have "
