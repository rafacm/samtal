...
MCP server and vinga-server the client, because the tools being
...
tools lives in `vinga_server.tools.device`.
...
def initialize_request(request_id: int, client_name: str, client_version: str) -> dict[str, Any]:
    ...
    The empty `vision` stanza is deliberate. vinga has no image
