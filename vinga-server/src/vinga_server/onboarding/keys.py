...
`vinga-server config ota-url`, which prints it to the operator's own
terminal.
"""
...
KEY_LABEL = b"vinga-onboarding-key-v1"
...
def onboarding_key(server: ServerConfig) -> str | None:
    ...
    has already refused the boot; `vinga-server config ota-url` runs
